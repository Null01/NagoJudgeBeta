...
@ManagedBean
@ViewScoped
public class SubmitViewBean implements Serializable {
    ...
    private final Logger logger = Logger.getLogger(SubmitViewBean.class);
        ...
        if (validationFailed) {
            ...
            try {
                ...
                if (this.problemMessage == null) {
                    HttpSession session = FacesUtil.getFacesUtil().getCurrentSession();
                    session.invalidate();
                    FacesUtil.getFacesUtil().printErrorResponse("Problema no identificado, contacte al administrador.");
                } else {
